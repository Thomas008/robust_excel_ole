...
A special feature of robust_excel_ole is that it allows to reopening books after closing them.
...
The option +:if_closed+ => +:hidden+ provokes that a closed book is opened unobtrusively in a separate Excel instance that is not visible and has no DisplayAlerts. Following closed books would be opened in this Excel instance as well with this option.
