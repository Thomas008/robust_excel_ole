...
RobustExcelOle helps controlling Excel. It is designed to manage simultaneously running Excel instances, even with simultanously happening user interactions.
...
RobustExcelOle works by sending VBA methods via Win32OLE. It keeps track of Excel instances and Excel workbooks.
...
1. +Excel.current+ or +Excel.new(:reuse => true)+ does not connect to an already running Excel instance, unless some workbook has been opened via RobustExcelOle. This has the consequence that so far there is not conflict management for blocking or unsavbed workbooks, but an error message would be returned. That is, the only option value for +:if_blocked+ and +:if_unsaved+ is +:raise+

2. Connecting to a hostname share path (e.g. "//DESKTOP-A3C5CJ6/spec/workbook.xls") does not work.

3. +Excel.close_all+ does not work yet. It relies on stepping through all heap objects in ObjectSpace. In jruby ObjectSpace is disabled. Each_object will only work with Class. The command +Excel.kill_all+ does work.

4. When providing an absolute path, you have to state the "/" ("C:/abc.xls" works, "C:abc" does not work).
