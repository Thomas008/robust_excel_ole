...
Another feature that RobustExcelOle povides is reopening Excel files after closing them. An Excel file is opened by default in the Excel instance where it was open before most recently. If this Excel instance is damaged or closed, then RobustExcelIle controls via options whether the Excel file is opened in the current (active) Excel instance, a new or a given Excel instance.
...
Moreover, RobustExcelOle allows unobtrusively reading and modifying Excel files, i.e. accessing Excel files without changing their "status". The status comprises whether the Excel file is open in some Excel instance , saved and writable. 
