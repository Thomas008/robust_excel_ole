...
It implements an Excel file management system.
...
Opening another workbook with the same file name in the running Excel. With the option +:if_unsaved+ => +:accept+  the workbook remains open if it contains unsaved changes.
...
The workbooks can be reopened after closing them. Example:

  book = Book.open('simple.xls')
  book.close
  reopened_book = Book.open('simple.xls')

robust_excel_ole provides identity transperence. This means that the same Book objects refer to the same Excel files, and vice versa.
...
Sometimes the user wants to read or modify a workbook, no matter if it is open in some Excel instance or not, is saved or unsaved. The method +unobtrusively+ provides this service. When opening a workbook unobtrusively, its status remains unchanged. This status includes, whether the book is opened or closed, saved or unsaved, readonly or writable. 

Options are +:if_closed+, +:read_only+, +:use_readonly_excel+, +:keep_open, +:visible+

Example: Open a book unobtrusively in a hidden Excel if the book is not open. The hidden Excel is an Excel instance that is not visible and has no DisplayAlerts. 
