...
* books are opened in an Excel instance if that one is already running (instead of opening Excel whenever a book is opened)
* book management system stores all books that have been open. This can be used, e.g., for reopening a book that has been closed before
...
+:if_obstructed+::   +:raise+ (default), +:forget+, +:save+, +:close_if_saved+, +:new_app+
...
Returns an open and writable book given its file name, if such a book exists, nil otherweise.
Returns the book that was opened most recently, if several open and writable books exist.

  book = Book.connect('simple.xls')
...
=== Makeing a book visible in the current Excel application.
...
=== Creating and reusing an Excel application.

Creating a new Excel.

  excel1 = Excel.create

Getting a running Excel application and reusing it.

  excel2 = Excel.current

Reusing a running Excel application, making it visible and turning on displayalerts.

   excel2 = Excel.new(:reuse => true, :visible => true, displayalerts => true).

Reusing a certain running Excel application.
...
Turning off or on Displayaerts of an Excel application.
...
=== Closing all Excel applications.
...
Opening the book in a new Excel application and make it visible.
