...
A sheet object can be accessed with a Book#[] method via an integer number or the sheet name.
...
Adding and appending an empty sheet.
...
Adding an empty sheet with a name before another sheet.
  ...
  book.add_sheet(:as => 'new_name', :before => another_sheet)
...
Copying a sheet after a another sheet and naming it.
  ...
  book.add_sheet(sheet, :as => 'new_name', :after => another_sheet)
