...
RobustExcelOle supports managing und processing Excel instances and Excel workbooks. It helps controlling Excel, and is designed to manage simultaneously running Excel instances,
...
1. Excel.current or Excel.new(:reuse => true) does not connect to an already running Excel instance, unless some workbook has been opened via RobustExcelOle. This has the consequence that there is not conflict management for blocking or unsavbed workbooks, but an error message would be returned. That is, the only option value for +:if_blocked+ and +:if_unsaved+ is +:raise+ 2. Excel.close_all does not work yet, but at least +kill_all+ does work. 3. When providing an absolute path, you have to state the "/" ("C:/abc.xls" works, "C:abc" does not work).
