...
RobustExcelOle allows a "script mode" and an "interactive mode": Commands enable to open Excel files in various Excel instances, enable to close, reopen, process and save the Excel files, without the need of the user's interaction, and even without the user noticing. While running this script, the user can open and process any Excel files at any time. RobustExcelOle manages the complex cases of conflicts that might occur such that the user does not need to interfere and the script can continue.

For example, suppose you want to process a list of Excel files. With help of RobustExcelOle, those Excel files can be rapidly opened, manipulated, closed and saved (script mode). Now assume, the Excel file "workbook.xls" is being processed, while the user has opened this Excel file, has manipulated but not saved it yet. Normally Excel would prompt a message and ask the user what to do. RobustExcelOle solves this conflict by using several options that control whether the changes of the user should be saved (accepted) or discarded (forgotten) before opening the workbook. 

  book1 = Book.open('workbook.xls')  # user
    ...
  book2 = Book.open('workbook.xls', :if_unsaved => accept) # script

Similarly, if the user has opened an Excel file that has the same name but a different path, the conflict is solved via options.

  book1 = Book.open('path1/workbook.xls')
    ...
  book2 = Book.open('workbook.xls', :if_obstructed => :forget)

RobustExcelOle provides the feature to reopen an Excel file after closing it. The Excel file is normally re-opened in the Excel instance where it was open before most recently. If this Excel instance is damaged or closed, then options manage whether the Excel file is opened in the current (active) Excel instance, a new or a given Excel instance.
...
Moreover, RobustExcelOle allows unobtrusively reading and modifying Excel files, i.e. accessing Excel files without changing their "status". The status comprises whether the Excel file is open or not, saved or not, and writable or not. 
...
+:if_unsaved+::      specify behaviour it the workbook was unsaved (+:raise+, +:forget+, +:accept+, +:alert+, +:new_excel+)
