...
Opening an book in a new Excel instance and make it visible.

  book = Book.open('simple.xls', :force_excel => :new, :visible => true)

Opening a book with the same name in a different path. 
The option +:if_obstructed+ => +:forget+ causes the old book to close such that it does not block the new book anymore.
...
Sometimes the user wants to read or modify a book, no matter if it is open in some Excel instance or not, is saved or unsaved. The method +unobtrusively+ provides this service. When opening a book unobtrusively, its status remains unchanged. This status includes, whether the book is opened or closed, saved or unsaved, readonly or writable. 
