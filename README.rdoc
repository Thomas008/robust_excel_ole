...
1. +Excel.current+ or +Excel.new(:reuse => true)+ does not connect to an already running Excel instance, unless some workbook has been opened via RobustExcelOle. This has the consequence that so far there is not conflict management for blocking workbooks. That is, the only option value for +:if_blocked+ is +:raise+
