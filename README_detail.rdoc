...
Options are +:reuse+ (+true+, +false+), +:visible+ (+true+, +false+), +:displayalerts+ (+true+, +false+, +:if_visible+) and +:calculation+.

The option +:calculation+ specifies the calculation mode of the Excel instance (+:manual+ (default) or +:automatic+). All workbooks opened in this Excel instance get this calculation mode.
