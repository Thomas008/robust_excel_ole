...
Options are +:reuse+ (+true+, +false+), +:visible+ (+true+, +false+), +:displayalerts+ (+true+, +false+, +:if_visible+) and +:calculation+ (+:manual+, +:automatic+).

The option +:calculation+ specifies the calculation mode of the Excel instance. All workbooks opened in this Excel instance get this calculation mode.
