...
+:calculation_mode   specify calculation mode of the Excel instance (+:manual+ (default) or +:automatic+)
...
The option +:calculation_mode+ can be +:automatic+ or +:manual+ (default). It specifies the calculation mode of the Excel instance. Since it cannot be set in an Excel instance without any workbooks, it is set here in the class +Workbook+. Note that if the calculation mode of the Excel instance is changed, also all other workbooks in this Excel instance get this calculation mode.  
