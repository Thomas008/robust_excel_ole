...
If a workbook is open and a workbook with same name but in a different path shall be opened, then the first workbook blocks opening the latter one. In this case:

+:raise+::          Raise an exception. Don't open the workbook.
+:forget+::         Close the blocking workbook, open the new workbook.
+:accept+::         Let the blocking workbook open.
