...
Now we define a name that refers to the range that shall consists of just the first cell, i.e. the 1st row and 1st column. Using VBA methods, you can use
...
You can (re-) define a name referring to a cell range with help of VBA methods by stating its name, and the row and the column of the cell.

  sheet.Names.Add("Name" => "name", "RefersToR1C1" => "Z1S1")

Using RobustExcelOle you can use

  sheet.add_name("name",1,1)
