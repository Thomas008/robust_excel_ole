...
We can copy and paste a range. The destination may be one cell in the same worksheet. With help of VBA methods you would do
...
You can also copy into another worksheet even of another workbook (but not in a different Excel instance), e.g.

  book = Workbook.open('spec/data/another_workbook.xls', :visible => true)
