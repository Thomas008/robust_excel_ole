...
You can copy and paste a range. For short, with "copy" we mean "copy and paste" here.

Let's assume, you have a source range

  src_range = sheet.range(1..2,3..5)

or, in A1-format,

  src_range = sheet.range("C1:E2")

To copy it, you provide a destination range of the same size.

  dest_range = sheet2.range(4..5,6..8)

or, in A1-format,
  
  dest_range = sheet2.range("F4:H5")

Now you can copy the values only by assigning the values with help of the VBA methods.

  dest_range.Value = src_range.Value

If you know about the destination range only the left top cell, e.g. [4,6], but don't want to care about the size of the source and destination range,
then you can use in this case

   dest_range = sheet2.range(4..4+src_range.Rows.Count,6..6+src_range.Columns.Count)

   dest_range.Value = src_range.Value

If you want to transpose the range you can use 
  
  dest_range = sheet2.range(4..6,6..7)
  dest_range.Value = dest_range.worksheet.workbook.excel.WorksheetFunction.Transpose(src_range)

If you want to copy not only the values but also formatting, cell formulas, etc., then you can use the VBA method Copy

   src_range.Copy(:destination => dest_range.ole_range)

or you use the RobustExcelOle method +copy+ by providing the destination address and the destination worksheet

   src_range.copy([4..5,6..8],sheet2)

The RobustExcelOle method +copy+ allows to copy a range not only into another worksheet and in another workbook, but also into another Excel instance. Note, that when copying into another Excel instance, the clipboard is being used. Furthermore, the destination range shall be a cell (a rectangular range destination is not being considered). 
