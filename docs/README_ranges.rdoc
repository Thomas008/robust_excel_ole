...
Now we copy and paste the range. With help of VBA methods you would do

  range.Copy(:destination => sheet.range([4..6,5..8]).ole_range)

or with help of RobustExcelOle

  range.copy_special([4,5])
...
You can also copy a range into another worksheet in another workbook.
  ...
  range.copy_special([4,5],book2.sheet(3))
...
You can copy and paste (for short, we use "copy" here) a range.
