...
Ranges containing rows or columns within the whole used range of a worksheet are defined, e.g., by setting the other parameter (columns or row) to +nil+:

  range = worksheet.range([1..3,nil])
  range = worksheet.range([nil,"A".."B"])

A row you can get by providing the row number, e.g.
...
You can get a row with help of   
