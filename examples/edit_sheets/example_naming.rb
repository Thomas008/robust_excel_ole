  ...
  Book.unobtrusively(extended_file_name) do |book|     
    book.each do |sheet|
      sheet.each do |cell_orig|
        ...
        #puts("first") if (contents && contents.is_a?(String))
        #puts("second") if contents && contents.is_a?(String)
        sheet.add_name(cell_orig.Row, cell_orig.Column, contents) if contents && contents.is_a?(String)
      end
    end
  end
  #book_orig.close
